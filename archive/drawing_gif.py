...
# === Configuratie ===
OUTPUT_FILENAME = "drawing.gif"
DRAWING_FILENAME = "drawing.png" # Zorg dat dit bestand in dezelfde map staat!
WIDTH = 600
HEIGHT = 100
DURATION_SECONDS = 30
PACKET_INTERVAL_SECONDS = 2
FPS = 15
DRAWING_SIZE = (24, 24) # De grootte waarnaar het tandwiel wordt geschaald
...
if not os.path.exists(DRAWING_FILENAME):
    print(f"FOUT: Het bestand '{DRAWING_FILENAME}' ontbreekt!")
    print("Plaats een PNG bestand van een tandwiel met transparante achtergrond")
    print("in dezelfde map als dit script en noem het 'drawing.png'.")
    sys.exit(1)

try:
    # Laad de drawing image en converteer naar RGBA voor transparantie ondersteuning
    drawing_source = Image.open(DRAWING_FILENAME).convert("RGBA")
    # Schaal het tandwiel naar een mooi formaat voor de lijn
    drawing_img = drawing_source.resize(DRAWING_SIZE, Image.Resampling.LANCZOS)
    drawing_w, drawing_h = drawing_img.size
    print(f"Tandwiel geladen en geschaald naar {drawing_w}x{drawing_h} pixels.")
...
print(f"Genereren van {TOTAL_FRAMES} frames met oranje lijn en tekeningen...")
...
for frame_num in range(TOTAL_FRAMES):
    ...
    for i in range(packets_started):
        ...
        if 0 <= frames_traveled <= travel_time_frames:
            ...
            paste_x = int(packet_center_x - drawing_w // 2)
            paste_y = int(packet_center_y - drawing_h // 2)
            
            # Plak het tandwiel plaatje op het huidige frame.
            # Het derde argument (drawing_img) wordt gebruikt als 'mask' 
            # om de transparantie van de PNG te behouden.
            img.paste(drawing_img, (paste_x, paste_y), drawing_img)
